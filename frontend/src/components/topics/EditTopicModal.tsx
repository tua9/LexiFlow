...
export function EditTopicModal({ open, topic, onClose, onSaved }: EditTopicModalProps) {
    ...
    const { updateTopic } = useTopics({ autoFetch: false });
    ...
    useEffect(() => {
        if (open && topic) {
            ...
            setColor(topic.color ?? TOPIC_COLORS[0].key);
            setError('');
        }
    }, [open, topic]);
    ...
    async function submit() {
        ...
        setSaving(true);
        try {
            await updateTopic({ id: topic.id, data: { name: name.trim(), color, isPublic, description: description.trim() } });
            onSaved();
            onClose();
        } catch (err) {
            console.error('[EditTopicModal] updateTopic', err);
    ...
    return (
        <Modal open={open} onClose={onClose} title="Chỉnh sửa chủ đề" size="sm">
